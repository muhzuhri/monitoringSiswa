...
<body>
    <div class="header">
        ...
        @if(isset($konfigurasi->header_3))
            <h2>{{ str_replace('{tahun}', ($siswa->tahunAjaran->tahun_ajaran ?? '2026/2027'), $konfigurasi->header_3) }}</h2>
        @endif
    </div>
    ...
    <table class="info-table">
        <tr>
            ...
            <td class="name1">{{ $siswa->nama }}</td>
            <td class="label">Pembimbing</td>
            <td class="separator">:</td>
            <td class="name">{{ $pembimbing->nama }}</td>
        </tr>
        <tr>
            <td class="label1">Nisn</td>
            <td class="separator1">:</td>
            <td class="name1">{{ $siswa->nisn }}</td>
            <td class="label">Periode</td>
            <td class="separator">:</td>
            <td class="name">
                @if($siswa->tgl_mulai_magang && $siswa->tgl_selesai_magang)
                    {{ \Carbon\Carbon::parse($siswa->tgl_mulai_magang)->translatedFormat('d F') }} -
                    {{ \Carbon\Carbon::parse($siswa->tgl_selesai_magang)->translatedFormat('d F Y') }}
                @else
                    -
                @endif
            </td>
        ...
        <tr>
            ...
            <td class="separator1">:</td>
            <td class="name1">{{ $siswa->jurusan }}</td>
            <td class="label">Lokasi Magang</td>
            <td class="separator">:</td>
            <td class="name">{{ $siswa->perusahaan }}</td>
        </tr>
    </table>
                        ...
                        <div style="display: table-row;">
                            ...
                            <div style="display: table-cell; vertical-align: middle;">
                                ...
                                <span class="fw-bold" style="margin-left: 15px; font-size: 14pt;">=
                                    {{ number_format($cumulative, 1) }}</span>
                            </div>
                        </div>
                    </div>
                </td>
        ...
        <tr>
            ...
            <td style="width: 50%; vertical-align: top; text-align: center;">
                <div class="signature-box" style="margin-top: 0; display: inline-block; text-align: center;">
                    ...
                    <p>{{ $latestPenilaian && $latestPenilaian->pemberi_nilai == 'Guru Pembimbing' ? 'Guru Pembimbing' : 'Pembimbing Siswa' }}
                    </p>
