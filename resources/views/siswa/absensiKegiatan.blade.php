                        ...
                        <div class="history-section">
                            <div class="ui-card history-card">
                                    ...
                                    <table class="ui-table">
                                        ...
                                        <tbody>
                                            @forelse($absensis as $absen)
                                                <tr>
                                                    <td>{{ \Carbon\Carbon::parse($absen->tanggal)->translatedFormat('D, d M Y') }}</td>
                                                    <td>
                                                        <div class="time-cell">
                                                            <span class="time-val in">{{ $absen->jam_masuk ? \Carbon\Carbon::parse($absen->jam_masuk)->format('H:i') : '-' }}</span>
                                                            @if($absen->foto_masuk)
                                                                <a href="{{ asset('storage/' . $absen->foto_masuk) }}" target="_blank" class="img-link" title="Lihat Foto Masuk">
                                                    ...
                                                    <td>
                                                        <div class="time-cell">
                                                            <span class="time-val out">{{ $absen->jam_pulang ? \Carbon\Carbon::parse($absen->jam_pulang)->format('H:i') : '-' }}</span>
                                                            @if($absen->foto_pulang)
                                                                <a href="{{ asset('storage/' . $absen->foto_pulang) }}" target="_blank" class="img-link" title="Lihat Foto Pulang">
                                                                    <i class="fas fa-image"></i>
                                                                </a>
                                                            @endif
                                                        </div>
                                                    ...
                                                    <td class="text-center">
                                                        @if($absen->verifikasi == 'verified')
                                                            <span class="badge-ui badge-success" title="Sudah diverifikasi pembimbing">
                                                                <i class="fas fa-check-circle me-1"></i> Disetujui
                                                            </span>
                                                        @elseif($absen->verifikasi == 'rejected')
                                                            <span class="badge-ui badge-danger" title="Ditolak: {{ $absen->keterangan }}">
                                                                <i class="fas fa-times-circle me-1"></i> Ditolak
                                                            </span>
                                                        @else
                                                            <span class="badge-ui badge-warning" title="Menunggu verifikasi pembimbing">
                                            ...
                                            @empty
                                                <tr>
                                                    <td colspan="4" style="text-align: center; color: #999; padding: 3rem;">Belum ada riwayat absensi.</td>
                                                </tr>
                                            @endforelse
                                        </tbody>
                                    </table>
                                ...
                                <div class="history-footer">
                                    <button class="btn-ripple btn-history-more" data-bs-toggle="modal" data-bs-target="#modalHistoryDetail">
                            ...
                            <form action="{{ route('siswa.logbook.store') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="kegiatan" class="form-control-custom" rows="5"
                                        placeholder="Apa yang kamu kerjakan hari ini? (Detailkan tugas, kendala, atau hasil kerja)"
                                        required></textarea>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="form-help-text">Lampirkan foto saat sedang mengerjakan tugas atau hasil kerja.</div>
                                </div>
                            ...
                            <div class="table-container">
                                <table class="ui-table">
                                    ...
                                    <tbody>
                                        @forelse($logbooks as $log)
                                            <tr>
                                                <td style="white-space: nowrap;">
                                                    <div class="fw-bold">{{ \Carbon\Carbon::parse($log->tanggal)->translatedFormat('D, d M Y') }}</div>
                                                </td>
                                                <td>
                                                    <div class="log-content-wrapper">
                                                        <div class="fw-bold text-dark">{{ Str::limit($log->kegiatan, 100) }}</div>
                                                        @if($log->catatan_pembimbing)
                                                            <div class="small mt-1 p-2 rounded bg-light border-start border-3 border-primary">
                                                                <i class="fas fa-comment-dots text-primary me-1"></i>
                                                                <span class="text-muted">Catatan:</span> "{{ $log->catatan_pembimbing }}"
                                                            </div>
                                                        @endif
                                                    </div>
                                                </td>
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="3" style="text-align: center; color: #999; padding: 3rem;">Belum ada riwayat logbook.</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
    ...
    <div class="modal fade" id="modalCheckIn" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content modal-content-custom">
                ...
                <div class="modal-footer modal-footer-custom">
                    <button type="button" class="btn-pill btn-pill-secondary" data-bs-dismiss="modal" style="background: #e9ecef; border: none; color: #495057;">Batal</button>
                    <button type="button" class="btn-pill btn-pill-primary" style="background: #198754; border: none; color: #ffffff;">Lakukan Check-In</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <script>
            ...
            async function fetchHistoryData() {
                ...
                const route = currentModalTab === 'absensi' ? '{{ route("siswa.absensi.detail") }}' : '{{ route("siswa.logbook.detail") }}';
                ...
                try {
                    ...
                    if (data.length === 0) {
                        tbody.innerHTML = `<tr><td colspan="${cols}" class="text-center p-5 text-muted"><i class="fas fa-folder-open mb-3 d-block" style="font-size: 2rem; opacity: 0.3;"></i> Tidak ada data di halaman ini.</td></tr>`;
                        return;
                    }
                    ...
                    if (currentModalTab === 'absensi') {
                        tbody.innerHTML = data.map(item => `
                            <tr>
                                <td><div class="date-col"><strong>${item.tanggal.split(',')[0]}</strong><span>${item.tanggal.split(',')[1]}</span></div></td>
                                <td>
                        ...
                        tbody.innerHTML = data.map(item => `
                            <tr>
                                ...
                                <td><span class="badge ${getLogStatusClass(item.status)}">${item.status}</span></td>
                            </tr>
                        `).join('');
                    }
                    ...
                    tbody.innerHTML = `<tr><td colspan="${cols}" class="text-center p-5 text-danger">Gagal memuat data.</td></tr>`;
                }
            }
